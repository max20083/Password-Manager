    ...
    with open("logs.txt","r+",encoding="utf-8") as f:
        ...
        for i in range(len(lines)):
            ...
            if web != "Выберите сайт":
                if web == web1:
                    if login != "" and password != "" and " " not in login and " " not in password:
...
def create_web():
    def add_web():
        ...
        if web != "" and web not in webs and " " not in web:
            webs.append(web)
...
login_input_but=Entry(frame_login_pass,width=15)
...
pass_input=Entry(frame_login_pass,width=15)
pass_input.pack(side=LEFT,padx=0,pady=0)
frame_create_acc=Frame(root)
frame_create_acc.pack()
create_acc_but=Button(frame_create_acc,text="Добавить учетную запись",width=20,command=create_acc)
